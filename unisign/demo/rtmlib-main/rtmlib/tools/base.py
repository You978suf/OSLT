...
class BaseTool(metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if backend == 'opencv':
            ...
            # Allow single provider strings/tuples or ordered provider lists.
            provider_list = providers if isinstance(providers,
                                                    list) else [providers]
            ...
            if device != 'cpu':
                expected_providers = [
                    p[0] if isinstance(p, tuple) else p for p in provider_list
                ]
                active_providers = self.session.get_providers()
                if not any(p in active_providers for p in expected_providers):
                    raise RuntimeError(
                        'Failed to initialize ONNX Runtime with non-CPU '
                        f'providers {expected_providers}. Active providers: '
                        f'{active_providers}.')
