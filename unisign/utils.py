...
try:
    ...
    HAS_DEEPSPEED = False
    print("Warning: DeepSpeed not installed. Training will run without DeepSpeed optimization.")
...
def init_deepspeed(args, model, optimizer, lr_scheduler):
    ...
    use_deepspeed = HAS_DEEPSPEED and args.zero_stage > 0
    if use_deepspeed:
        ...
        print("DeepSpeed disabled (zero_stage=0), using standard training...")
        _wrapped_model = model
        _optimizer = optimizer
        _lr_sched = lr_scheduler
...
def get_args_parser():
    ...
    # select dataset
    parser.add_argument("--dataset", default="CSL_Daily", choices=['CSL_News', "CSL_Daily", "WLASL", "How2Sign", "OpenASL", "OSL-Words", "OSL-Sentences", "OSL-Words-Webcam"])
